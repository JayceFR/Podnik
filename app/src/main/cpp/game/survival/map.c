        ...
        for (int cx=0; cx<WORLD_W; cx++){
            ...
            if (dirtCount > 0) {
                ...
                if (hashFind(computers, buffer) == NULL){
                    hashSet(computers, buffer, create_dynarray((dynarray_freef)computerFree, NULL));
                }
            ...
            for (int y=0; y<CHUNK_SIZE; y++){
                for (int x=0; x<CHUNK_SIZE; x++){
                    ...
                    //         hashSet(enemies, buffer, create_dynarray((dynarray_freef)enemyFree, NULL));
                    //     }
...
void placeProperty(hash map, hash offgridTiles, Texture2D prop, int index, int x, int y) {
    ...
    if (hashFind(offgridTiles, buffer) == NULL){
        dynarray arr = create_dynarray((dynarray_freef)offgridTileFree, NULL);
...
mapData mapCreate(hash offgridTiles, BIOME_DATA biome_data, Texture2D pathDirt, int level) {
   ...
   mapData data; 
   data.map = hashCreate((hashprintfunc)tilesPrint, (hashfreefunc)tilesFree, NULL);
   ...
   data.enemies = hashCreate(NULL, (hashfreefunc)enemyHashFree, NULL);
   data.computers = hashCreate(NULL, (hashfreefunc)computerHashFree, NULL);
