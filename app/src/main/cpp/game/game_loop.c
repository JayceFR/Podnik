...
void updateBoids(hash flockGrid, Vector2 *averageVels){
    dynarray toChange = create_dynarray((dynarray_freef)freeToChange, (dynarray_printf)printToChange);
...
dynarray GetWalkableTiles(hash map) {
    dynarray list = create_dynarray((dynarray_freef)free, NULL);
...
int GameMain() {
    ...
    hash flockGrid = hashCreate(NULL, (hashfreefunc)free_dynarray, NULL);
    dynarray allBirds = create_dynarray((dynarray_freef)free, NULL);
    ...
    dynarray projectiles = create_dynarray((dynarray_freef)projectileFree,NULL);
    dynarray eprojectiles = create_dynarray((dynarray_freef)projectileFree, NULL);

    hash offgridMap = hashCreate(NULL, (hashfreefunc)offgridsFree, NULL);
    dynarray offgrids;
            ...
            if (transitionType == TT_LEVEL) {
                ...
                if (!transitionPhaseLoad) {
                    ...
                    if (transitionRadius >= transitionMaxRadius) {
                        ...
                        allBirds = create_dynarray((dynarray_freef)free, NULL);
                        flockGrid = hashCreate(NULL, (hashfreefunc)free_dynarray, NULL);
                        data->flockGrid = flockGrid;

                        offgridMap = hashCreate(NULL, (hashfreefunc)offgridsFree, NULL);
